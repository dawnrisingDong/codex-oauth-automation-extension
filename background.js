...
function normalizeLoopbackHostToLocalhost(rawUrl) {
  const parsed = parseUrlSafely(rawUrl);
  if (!parsed) return '';
...
async function requestStep8LocalhostCallback(rawUrl) {
  if (!isLocalhostOAuthCallbackUrl(rawUrl)) {
    throw new Error('步骤 8：捕获到的 localhost OAuth 回调地址无效，无法继续请求。');
  }

  const requestUrl = normalizeLoopbackHostToLocalhost(rawUrl);
  if (!isLocalhostOAuthCallbackUrl(requestUrl)) {
    throw new Error('步骤 8：规范化 localhost 回调地址失败，请重试步骤 8。');
  }
  ...
  try {
    ...
    throw new Error(`步骤 8：请求 localhost 回调地址失败（${requestUrl}）。${getErrorMessage(error)}`);
  } finally {
...
async function closeStep8LoopbackCallbackTabs(rawUrl) {
  const parsed = parseUrlSafely(rawUrl);
  if (!parsed || parsed.hostname !== '127.0.0.1') return 0;
  return closeLocalhostCallbackTabs(rawUrl);
}
...
function isEmailAutoRetryFromStep1Error(error) {
  const message = String(typeof error === 'string' ? error : error?.message || '');
  return /EMAIL_AUTO_RETRY_FROM_STEP1::/.test(message);
}
...
const EMAIL_FETCH_RETRY_WAIT_MS = 30 * 1000;
...
async function ensureAutoEmailReady(targetRun, totalRuns, attemptRuns) {
  ...
  await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮：邮箱未就绪，30 秒后将从第 1 步自动重试本轮 ===`, 'warn');
  await broadcastAutoRunStatus('retrying', {
    currentRun: targetRun,
    totalRuns,
    attemptRun: attemptRuns,
  });
  await sleepWithStop(EMAIL_FETCH_RETRY_WAIT_MS);
  throw new Error('EMAIL_AUTO_RETRY_FROM_STEP1::先自动获取邮箱，或手动粘贴邮箱后再继续');
}
...
async function autoRunLoop(totalRuns, options = {}) {
  ...
  while (successfulRuns < totalRuns && attemptRuns < maxAttempts) {
    ...
    try {
      ...
      if (isEmailAutoRetryFromStep1Error(err)) {
        await addLog(`目标 ${targetRun}/${totalRuns} 轮邮箱未就绪，已触发自动重试：下一次尝试将从第 1 步开始。`, 'warn');
        cancelPendingCommands('邮箱未就绪，准备从第 1 步自动重试。');
        await broadcastStopToContentScripts();
        await broadcastAutoRunStatus('retrying', {
          currentRun: targetRun,
          totalRuns,
...
async function executeStep8(state) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const finalizeStep8Callback = (callbackUrl) => {
      ...
      requestStep8LocalhostCallback(callbackUrl).then((requestUrl) => {
        if (requestUrl !== callbackUrl) {
          return addLog(`步骤 8：检测到 127.0.0.1 回调，已改为 localhost 并请求：${requestUrl}`, 'info').then(() => requestUrl);
        }
        return requestUrl;
      }).then(async (requestUrl) => {
        try {
          const closedCount = await closeStep8LoopbackCallbackTabs(callbackUrl);
          if (closedCount > 0) {
            await addLog(`步骤 8：已关闭点击“继续”后触发的 127.0.0.1 回调标签页（${closedCount} 个）。`, 'info');
          }
        } catch (error) {
          await addLog(`步骤 8：清理 127.0.0.1 回调标签页失败，将继续流程。${getErrorMessage(error)}`, 'warn');
        }
        return requestUrl;
      }).then((requestUrl) => {
        return addLog(`步骤 8：已捕获 localhost 地址：${requestUrl}`, 'ok').then(() => requestUrl);
      }).then((requestUrl) => {
        return completeStepFromBackground(8, { localhostUrl: requestUrl });
      }).then(() => {
        resolve();
      }).catch((err) => {
        reject(err);
